...
export class InputEngine {
	...
	constructor(mesh: AbstractMesh, onChanged: () => void) {
		...
		this.originalRotation =
			mesh.rotationQuaternion ?? Quaternion.FromEulerVector(new Vector3(0, 0, 0));
	}
		...
		switch (this.buttonState) {
			...
			case ButtonClickState.right:
				this.mesh.rotationQuaternion = new Quaternion(
					(this.startPointerY - ev.clientY) * 0.005,
					(this.startPointerX - ev.clientX) * 0.005,
					0,
					1,
				)
					.multiply(this.originalRotation)
					.normalize();
	...
	pointerdown_handler(ev: PointerEvent) {
		...
		if (this.evCache.length == 1) {
			...
			this.startPointerX = this.lastPointerX =
				(this.evCache[2].clientX + this.evCache[1].clientX + this.evCache[0].clientX) / 2;
			this.startPointerY = this.lastPointerY =
				(this.evCache[2].clientY + this.evCache[1].clientY + this.evCache[0].clientY) / 2;
		}
	...
	pointermove_handler(ev: PointerEvent) {
		...
		if (this.evCache.length == 1) {
			...
			this.mesh.rotationQuaternion = new Quaternion(
				(this.startPointerY - centerY) * 0.005,
				(this.startPointerX - centerX) * 0.005,
				0,
				1,
			)
				.multiply(this.originalRotation)
			...
			if (this.prevDiff > 0) {
				...
				this.mesh.scaling = this.mesh.scaling.add(new Vector3(scale, scale, scale));
			}
